...
1. **Auto-increment**: SQLite uses `AUTOINCREMENT`, PostgreSQL uses `SERIAL`
2. **Timestamps**: SQLite uses `DATETIME`, PostgreSQL uses `TIMESTAMPTZ`
3. **Date functions**: `julianday()` in SQLite → `EXTRACT(EPOCH FROM ...)` in PostgreSQL
4. **GROUP_CONCAT**: SQLite's `GROUP_CONCAT` → PostgreSQL's `STRING_AGG()`

## Next Steps

After running migrations, you'll need to update API endpoints to use Supabase query builder instead of raw SQL. See example conversions below.
